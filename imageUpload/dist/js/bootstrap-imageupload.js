...
(function($) {
    ...
    function isValidImageFile(file, callback) {
        ...
        {
            callback(false, 'Gambar terlalu besar (max ' + options.maxFileSizeKb + 'kB).');
            return;
        }
        ...
        if ($.inArray(fileExtension, options.allowedFormats) > -1) {
            callback(true, 'Image file diizinkan.');
        }
        else {
            callback(false, 'Jenis file ini tidak diizinkan.');
        }
    }
    ...
    function resetFileTab($fileTab) {
        ...
        $fileTab.find('.btn span').text('Pilih Gambar');
    ...
    function submitImageFile($fileTab) {
        ...
        $fileTab.find('.alert').remove();
        $fileTab.find('img').remove();
        $browseFileButton.find('span').text('Pilih Gambar');
        ...
        isValidImageFile(file, function(isValid, message) {
            if (isValid) {
                ...
                fileReader.onload = function(e) {
                    ...
                    $browseFileButton.find('span').text('Ganti');
